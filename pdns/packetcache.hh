...
{
protected:
  static uint32_t canHashPacket(const std::string& packet, char* metadata, bool skipECS=true)
  {
    uint32_t ret = 0;
    ...
    if (skipBegin > p) {
      // cerr << "1-Hashing from " << (p-packet.c_str()) << " for " << skipBegin-p << "bytes, end is at "<< end-packet.c_str() << endl;
      ret = burtle((const unsigned char*)p, skipBegin-p, ret);
    }
    if (skipEnd < end) {
      // cerr << "2-Hashing from " << (skipEnd-packet.c_str()) << " for " << end-skipEnd << "bytes, end is at " << end-packet.c_str() << endl;
      ret = burtle((const unsigned char*) skipEnd, end-skipEnd, ret);
    }

    size_t meta_data_len = strlen(metadata);
    if (meta_data_len > 0) {
      ret = burtle((const unsigned char*) metadata, meta_data_len, ret);
    }
